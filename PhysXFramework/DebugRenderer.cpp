...
void DebugRenderer::InitRenderer(ID3D11Device* pDevice, UINT bufferSize)
{
	...
	m_pEffect = ContentManager::GetInstance()->Load<ID3DX11Effect>(L"Resources/Effects/DebugRenderer.fx");
	m_pTechnique = m_pEffect->GetTechniqueByIndex(0);
	m_pWvpVariable = m_pEffect->GetVariableBySemantic("WORLDVIEWPROJECTION")->AsMatrix();

	if (!m_pWvpVariable->IsValid())
		Logger::GetInstance()->LogWarning(L"Debug Renderer: Invalid Shader Variable! (WVP)");
...
void DebugRenderer::CreateVertexBuffer(ID3D11Device* pDevice)
{
	...
	if (m_FixedBufferSize > 0 && m_pVertexBuffer != nullptr)
	{
...
void DebugRenderer::CreateFixedLineList()
{
	...
	//@END!
	m_FixedBufferSize = static_cast<UINT>(m_FixedLineList.size());
}
...
void DebugRenderer::DrawGrid(const SceneContext& sceneContext)
{
	if (!m_RendererEnabled)
		return;
	...
	const UINT regularSize = static_cast<UINT>(m_LineList.size());
	const UINT dynamicSize = static_cast<UINT>(regularSize) + pxBuffSize;
	...
	{
		Logger::GetInstance()->LogDebug(L"DebugRenderer::Draw() > Increasing Vertexbuffer Size!");
		m_BufferSize = dynamicSize;
		CreateVertexBuffer(Locator::GetD3D11()->pDevice);
	}

	auto pDevContext = Locator::GetD3D11()->pDeviceContext;
	...
	auto& viewProj = sceneContext.GetCamera()->GetViewProjection();
